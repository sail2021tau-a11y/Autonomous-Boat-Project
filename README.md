...
- **The `stop` command is correctly sent on task completion and on operator kill commands, but not yet on a navigation task's own detection-loss timeout.** Tasks 3 and 4 publish `stop` automatically on reaching their target, and every task's `*_kill_ros2.sh` script publishes `stop` when the operator issues the matching `K` command — both work as intended. However, each navigation node's `TimerManager` (Tasks 1–3), which is meant to end the task if the camera stops detecting valid targets for an extended period, currently only shuts its own ROS 2 node down in `timer_callback()` — it does not publish `stop` first, so the engine controller keeps outputting the last commanded thrust instead of stopping. Task 1's timer is also currently disabled in code (its `start_timer(TIMEOUT)` call is commented out). Do not rely on this detection-loss timeout as a safety mechanism until it publishes `stop`.
...
- The Arduino sketch has no RC override and no soft-start ramping (soft-start was validated separately during hardware stability testing but was not carried into this branch's firmware). A navigation task's own detection-loss timeout does not currently publish `stop` before shutting the node down, so it does not actually halt the motors as intended, and Task 1's timeout is disabled in code — see [Safety Notes](#safety-notes) for the exact gap.
